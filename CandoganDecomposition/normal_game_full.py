...
class GameFull():
    def __init__(self, num_strategies_for_player, **kwargs):
        ...
        # Make inner products
        self.make_inner_products()


        # ------------------------------------------------
        # Now the six following Metric instances are available:
        # ------------------------------
        # self.metric_0, metric in C_0
        # self.metric_1, metric in C_1
        # self.metric_0N, metric in (C_0)^N

        # self.metric_0_up, metric in C^0
        # self.metric_1_up, metric in C^1
        # self.metric_0N_up, metric in (C^0)^N
        # ------------------------------
        # and  can be called to build pinv
        ...
        # ------------------------------------------------
        # Generalized pinv enters in two positions: in game space for normalization, and in flow space for exact projection
        # Both are used in the decomposition
        # ------------------------------------------------
        self.pwc_homomorphism = metric.Homomorphism( self.pwc_matrix, self.metric_0N_up, self.metric_1_up )
        self.pwc_matrix_pinv = self.pwc_homomorphism.pinv

        # pwc_matrix_pinv built with default npla.pinv(self.pwc_matrix) or with my method agree if metric type is euclidean
        # ------------------------------------------------
        ...
        #------------------------------------
        # generalized
        self.coboundary_0_homomorphism = metric.Homomorphism( self.coboundary_0_matrix, self.metric_0_up, self.metric_1_up )
        self.coboundary_0_matrix_pinv = self.coboundary_0_homomorphism.pinv

        # coboundary_0_matrix_pinv agree in npla.pinv(self.coboundary_0_matrix) case and my Homomorphism generalized case with Euclidean metric
        ...
        # -----------------------------------------------------------------------------------------------
        # Move here init of GameFull class
        # -----------------------------------------------------------------------------------------------
        ...
        #     # build C_1 (down!) metric and C0N metric using harmonic measure
        ...
        #     #--------------------------------------------------------
        #     # Build harmonic C_1 inner product (down; metric in C^1 is inverse)
        #     #--------------------------------------------------------
        #     # The fact that I'm using metric down is clear in def below (here in pseudo-code) of adjoint of coboundary 0 map
        #     #(d_0)+ : C^1 --> C^0
        #     # Adjoint of d_0: C^0 --> C^1
        #     # Flat and sharp are wrt metrics down, in C_0 and C_1, because 
        #     # flat in a vector space goes out (from vector space to dual)
        #     # sharp in a vecrtor space goes in (from dual to vector space)
        #     # Hence defining
        #     #--------------------------------------------------------
        #     # delta_0_adjoint_matrix = metric_0.flat_matrix * boundary_1_matrix * metric_1.sharp_matrix
        #     #--------------------------------------------------------
        #     # means I'm going first in 1, then boundary_1, then out 0 - and this is consistent if the metrics are in C_0 and C_1.

        #     # So,
        #     # metric in C_1 (down) = diag (1 / sqrt( measure_edge) )
        #     # metric in C^1 (up)   = diag ( sqrt( measure_edge) )
        #     #--------------------------------------------------------
        ...
        #     # Diagonal with measure_player times measure_pure_profile
    ...
    # -----------------------------------------------------------------------------------
    def make_inner_products(self):
        ######################################################################
        # INNER PRODUCT
        # metric_0, metric_1, metric_0N are Metric instances for C_0, C_1, (C_0)^N DOWN
        ######################################################################
        if self.metric_type == 'euclidean':
            self.metric_0 = metric.Metric( metric.EuclideanMetric(self.dim_C0).matrix )
            self.metric_1 = metric.Metric( metric.EuclideanMetric(self.dim_C1).matrix )
            ...
            self.harmonic_measure = self.kwargs['harmonic_measure']
            assert len(self.harmonic_measure) == len(self.num_strategies_for_player), "harmonic measure has bad size"
            for i, m in enumerate(self.harmonic_measure):
                assert len(m) == self.num_strategies_for_player[i], "harmonic measure has bad size"
            ...
            for i, x in enumerate(C0N_measures):
                # Not sure; this may be reciprocal (1/all), partial reciprocal (mu_i / mu_a rather than mu_i * mu_a), square root, .... to figure out
                # g0N[i][i] = C0N_measures[i]
                g0N[i][i] = 1 / np.sqrt(  C0N_measures[i] ) 
        ...
        # In all cases
        self.metric_0_up = metric.Metric(  self.metric_0.inverse_matrix  )
        self.metric_1_up = metric.Metric(  self.metric_1.inverse_matrix  )
        self.metric_0N_up = metric.Metric(  self.metric_0N.inverse_matrix  )
        #--------------------------------------------------------
    ...
    # -----------------------------------------------------------------------------------------------
    # Move here methods of GameFull class
    # -----------------------------------------------------------------------------------------------
    def slice(self, a, i):
        return a[:i]+a[i+1:]

...
Includes generalisation to non-Euclidan metrics'''

DIAGONAL_SHIFT_0 = 1
DIAGONAL_SHIFT_1 = 3
LATEX_PLOT_POTENTIAL = False
# MAKE_HARMONIC_PAYOFF = True
...
class Payoff():
    ...
    def __init__(self, game, payoff_vector = 0, value = 1):
    ...
    ########################################################
    # Works but deprecated: manual input
    ########################################################
    # def input_payoff_function(self):
    #     """
    #     Manual input
    #     The output is a dictionary working like the payoff function u: A --> R^N
    #     So u[a] = (u_1(a), ..., u_N(a)) for a in curly_A, that is for a in game.strategy_profiles
    ...
    #         po = []
    #         for pl in self.game.players:
    #             x = input(f'payoff for player {pl.player_name} for strategy profile {strategy_profile}? ')
    #             po.append(float(x))
    ...
    # def make_payoff_dict_from_payoff_function(self):
    #     """
    #     The output is a dictionary assigning the payoff to each player and strategy profile, taking as input elements of game.payoff_basis 
    #     So u[e] = u_i(a) for e = (i,a) in game.payoff_basis
    #     It is true that self.payoff_dict[e] == self.payoff_function[a][i] for e = (i,a)
    #     """
    #     payoff_dict = {}
    #     for payoff_basis_vector in self.game.payoff_basis:
    #         player_name, strategy_profile = payoff_basis_vector
    #         payoff = self.payoff_function[strategy_profile][player_name]
    #         payoff_dict[tuple(payoff_basis_vector)] = payoff
    ...
    def print_NE(self):
        if self.game.num_players != 2:
            print(f'At the moment can find NE only for bimatrix games')
            return
    ...
    def fix_edges_orientation(self):
        """Re-orient edges such that all actor's deviations are positive"""
    ...
    def decompose_payoff(self):
        ...
        if self.game.metric_type != 'euclidean':
    ...
    def measure_potentialness(self):
        ...
        '''
        uP_norm = (self.uP.T * self.uP)[0,0]
        uH_norm = (self.uH.T * self.uH)[0,0]
    ...
    def make_latex_graph_2x2_code(self, potential = False):
        ...
        def get_payoff(node):
            ...
            return f'{nice(round(a,1))},{nice(round(b,1))}'
        ...
        if potential:
            nodes_latex_code = f"""
    \x5cnode[main, label={{left:\x5csmall \x5ctextbf{{\x5ctextcolor{{teal}}{{[{nice(round(self.potential[0],1))}]}}}}({get_payoff((1,1))}) }}] (1) {{$11$}};
    \x5cnode[main, label={{right:\x5csmall \x5ctextbf{{\x5ctextcolor{{teal}}{{[{nice(round(self.potential[1],1))}]}}}}({get_payoff((1,2))})}}] (2) [right of=1] {{$12$}};
    \x5cnode[main, label={{left:\x5csmall \x5ctextbf{{\x5ctextcolor{{teal}}{{[{nice(round(self.potential[2],1))}]}}}}({get_payoff((2,1))})}}] (3) [above of=1] {{$21$}};
    \x5cnode[main, label={{right:\x5csmall \x5ctextbf{{\x5ctextcolor{{teal}}{{[{nice(round(self.potential[3],1))}]}}}}({get_payoff((2,2))})}}] (4) [above of=2] {{$22$}}; 
            """
    ...
    def make_latex_graph_2x3_code(self, potential = False):
        ...
        if potential:
            nodes_latex_code = f"""
    \x5cnode[main, label={{[xshift=-1.5cm, yshift=-1cm]\x5csmall \x5ctextbf{{\x5ctextcolor{{teal}}{{[{round(self.potential[0],1)}]}}}}({get_payoff((1,1))}) }}] (1) {{$11$}};
    \x5cnode[main, label={{[xshift=-0.0cm, yshift=-1.4cm]\x5csmall \x5ctextbf{{\x5ctextcolor{{teal}}{{[{round(self.potential[1],1)}]}}}}({get_payoff((1,2))})}}] (2) [right of=1] {{$12$}};
    \x5cnode[main, label={{[xshift=+1.5cm, yshift=-1cm]\x5csmall \x5ctextbf{{\x5ctextcolor{{teal}}{{[{round(self.potential[2],1)}]}}}}({get_payoff((1,3))})}}] (3) [right of=2] {{$13$}};
    \x5cnode[main, label={{[xshift=-1.5cm, yshift=-1cm]\x5csmall \x5ctextbf{{\x5ctextcolor{{teal}}{{[{round(self.potential[3],1)}]}}}}({get_payoff((2,1))})}}] (4) [above of=1] {{$21$}};
    \x5cnode[main, label={{[xshift=-0.0cm, yshift=-0.05cm]\x5csmall \x5ctextbf{{\x5ctextcolor{{teal}}{{[{round(self.potential[4],1)}]}}}}({get_payoff((2,2))})}}] (5) [right of=4] {{$22$}}; 
    \x5cnode[main, label={{[xshift=+1.5cm, yshift=-1cm]\x5csmall \x5ctextbf{{\x5ctextcolor{{teal}}{{[{round(self.potential[5],1)}]}}}}({get_payoff((2,3))})}}] (6) [right of=5] {{$23$}};
            """
...
class Game():
    ...
    Game class
    """
    def __init__(self, num_strategies_for_player, metric_type = 'euclidean', manual_metric_generators = 0):
        ...
        assert(metric_type in ['euclidean', 'random', 'manual', 'diagonal'])
        self.metric_type = metric_type
        self.manual_metric_generators = manual_metric_generators
        ...
        #(δ_0)* : C_1 --> C_0
        # Dual of δ_0: C^0 --> C^1
        self.boundary_1_matrix = self.make_boundary_1_matrix()
        ######################################################################

        ######################################################################
        # MATRIX coboundary 0 map
        # δ_0: C^0 --> C^1
        self.coboundary_0_matrix = self.boundary_1_matrix.transpose()
        ######################################################################
    ...
    def verbose_game(self):
        ...
        print(f'δ_0: {self.dim_C0} --> {int(self.dim_C1)}')
        print(f'δ_1: {int(self.dim_C1)} --> {int(self.dim_C2)}')
        ...
        print(f'dim P = dim Im δ_0 = {self.dim_P}')
